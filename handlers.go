...
func (app *App) eventEditData(r *http.Request, event *Event) map[string]any {
	...
	if event.EventType == "attendance" {
		...
		for k, v := range app.santaAdminData(event) {
			data[k] = v
		}
	} else {
...
// handleAPIEventEmailPreview renders the magic-link email twice (FR + EN)
// using the current form state — including unsaved overrides and the
// live description — so the admin sees exactly what a participant would
// receive. Driven from admin.js on every form change (debounced).
func (app *App) handleAPIEventEmailPreview(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, `{"error":"method not allowed"}`, 405)
		return
	}
	...
	e := Event{
		...
		TitleFR:           req.TitleFR,
		TitleEN:           req.TitleEN,
		DescriptionFR:     sanitizeEventDescription(req.DescriptionFR),
		DescriptionEN:     sanitizeEventDescription(req.DescriptionEN),
	...
	base := baseURLFor(r)
	pFR := SantaParticipant{FirstName: "Marie"}
	pEN := SantaParticipant{FirstName: "Mary"}
	_, frHTML := renderSantaLinkEmail("fr", pFR, e, base+"/santa/edit?token=preview")
	_, enHTML := renderSantaLinkEmail("en", pEN, e, base+"/santa/edit?token=preview")
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"fr": frHTML, "en": enHTML})
}
...
func (app *App) handleAdminRegistrations(w http.ResponseWriter, r *http.Request) {
	...
	if event.EventType == "secret_santa" {
		http.Redirect(w, r, fmt.Sprintf("/admin/event/edit?id=%d&lang=%s", id, LangFromRequest(r)), http.StatusSeeOther)
		return
	}
...
func (app *App) handleSantaRegister(w http.ResponseWriter, r *http.Request) {
	...
	messageID, err := app.Email.Send(r.Context(), p.Email, subject, htmlBody, invitationAttachments()...)
	if err != nil {
...
func (app *App) handleAdminSantaParticipantDelete(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, fmt.Sprintf("/admin/event/edit?id=%d&lang=%s", eventID, lang), http.StatusSeeOther)
}
